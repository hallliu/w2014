...
void *statistical_worker (void *_data) {
    ...
    while (1) {
        ...
        if (lock->try_lock (lock)) {
            ...
            source_pdf[queue_num] = 0.95 * source_pdf[queue_num] + 0.05 * pkt->iterations;
...
void *statistical_worker1 (void *_data) {
    struct thread_data *data = (struct thread_data *) _data;
    int queue_num;
    int queue_runs = 0;
    Packet_t *pkt = NULL;
