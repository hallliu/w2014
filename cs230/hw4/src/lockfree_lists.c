#include <stdlib.h>
#include <stdio.h>
#include <stdbool.h>

#include <packetsource.h>

#include "lists.h"

#define _UNUSED_ __attribute__((unused))
// Lookup table for bit-reversals copy-pasted from http://stackoverflow.com/a/746203
static char reverses[256] = {  };

static inline unsigned int rev_bits(int x) {
    return  ( (reverses[(x >> 24)])
            | (reverses[(x >> 16) & 0xff] << 8)
            | (reverses[(x >> 8) & 0xff] << 16)
            | (reverses[x & 0xff] << 24)
            );
}

struct lockfree_list *create_lockfree_list(void) {
    return NULL;
}

int lf_add(struct lockfree_list *l _UNUSED_, int key _UNUSED_, Packet_t *pkt _UNUSED_) {
    return 0;
}

bool lf_remove (struct lockfree_list *l _UNUSED_, int key _UNUSED_) {
    return false;
}

bool lf_contains (struct lockfree_list *l _UNUSED_, int key _UNUSED_) {
    return false;
}

void destroy_lockfree_list (struct lockfree_list *l _UNUSED_) {
    return;
}
