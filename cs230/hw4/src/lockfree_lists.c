...
static unsigned char reverses[256] = {  0x00, 0x80, 0x40, 0xC0, 0x20, 0xA0, 0x60, 0xE0, 0x10, 0x90, 0x50, 0xD0, 0x30, 0xB0, 0x70, 0xF0, 
...
static inline unsigned rev_bits(unsigned x) {
    return  ( ((unsigned) reverses[(x >> 24)] & 0xff)
            | ((unsigned) reverses[(x >> 16) & 0xff] << 8)
            | ((unsigned) reverses[(x >> 8) & 0xff] << 16)
            | ((unsigned) reverses[x & 0xff] << 24)
            );
}
